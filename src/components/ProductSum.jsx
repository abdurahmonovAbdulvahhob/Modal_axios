...
const ProductSum = ({ pro }) => {
  ...
  const oldPrice = () => {
    if (pro.price < 600) {
      return (
        <span className="text-sm text-slate-900 line-through">{`$${Math.round(
          pro.price * 1.5
        ).toFixed()}`}</span>
      );
    }
  ...
  return (
    <div className="mt-2 mb-5 flex gap-2 items-center justify-between flex-col">
      <p>
        <span className="text-3xl font-bold text-slate-900">{`$${pro.price}`}</span>
        {oldPrice()}
      </p>
      <div className="flex">
        {stars}
